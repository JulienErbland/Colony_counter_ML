...
def load_counter_model(name, weights_path):
    if name not in MODEL_DICTIONARY:
        raise ValueError(f"Unknown counting model '{name}'")

    entry = MODEL_DICTIONARY[name]
...
def evaluate_full_pipeline(
    ...
    if "aug" in filename or "augmented" in filename:
        output = "pipeline_results_augmented.csv"
    else:
        output = output_csv
    ...
    if len(valid_df) > 0:
        ...
        if ss_tot == 0:
            # All ground truth values identical -> R^2 undefined
            r2_score = None
        else:
...
if __name__ == "__main__":
    ...
    # Classifier always uses CountabilityClassifier entry in MODEL_DICTIONARY
    parser.add_argument("--classifier_weights", default=MODEL_DICTIONARY["CountabilityClassifier"]["weights"])

    # Dynamically offer all counting models from MODEL_DICTIONARY (except the classifier)
    available_counter_models = [m for m in MODEL_DICTIONARY.keys() if m != "CountabilityClassifier"]

    parser.add_argument(
        "--counter_model",
        default="ResNet34",
        choices=available_counter_models,
        help="Which counting model to evaluate.",
    )

    # User may override weights; if None, use dictionary default
    parser.add_argument(
        "--counter_weights",
        default=None,
        help="Optional path to custom counter model weights. If not provided, the model default is used."
    )
    ...
    #Select counter weights
    if args.counter_weights is None:
        # use default from dictionary
        counter_weights = MODEL_DICTIONARY[args.counter_model]["weights"]
        print(f"Using DEFAULT weights for {args.counter_model}: {counter_weights}")
    else:
        counter_weights = args.counter_weights
        print(f"Using CUSTOM counter weights: {counter_weights}")

    evaluate_full_pipeline(
        csv_path=args.csv,
        classifier_weights=args.classifier_weights,
        counter_model_name=args.counter_model,
        counter_model_weights=counter_weights,
