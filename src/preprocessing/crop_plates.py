...
# If the crop keeps at least this fraction (75%) of the original area,
# we assume the image is already cropped and we do not crop further.
NO_CROP_MIN_AREA_RATIO = 0.75
# -----------------------------------------------------------
...
def crop_with_fallback(img, calibrated_rect_norm=None):
    ...
    # --- NO-OP CROPPING LOGIC ---
    # If the detected crop is almost the full image, assume it's already cropped.
    full_area = W * H
    crop_area = w * h
    if full_area > 0:
    ...
    if area_ratio >= NO_CROP_MIN_AREA_RATIO:
        # Do not modify the image; keep previous calibration if any.
        return img, calibrated_rect_norm, True
...
def process_folder(src_root: Path, dst_root: Path):
    """
    Crop all the plates images found inside this folder
    
    return False if some images failed detection
    """ 
    ...
    print("\n" + "="*50)
    if fallback_list:
        print(f"SUMMARY (CROP PLATE): {len(files) - len(fallback_list)}/{len(files)} images SUCCESSFULLY cropped !")
        ...
        print("SUMMARY (CROP PLATE): Perfect run! All images were confidently detected.")
        print("="*50 + "\n")
        return True
