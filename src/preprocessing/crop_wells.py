...
def crop_wells(file_path, plate_type, base_dir, debug=False, save_files=True):
...
def crop_wells_script(root_dir, base_dir):
    """
    if crop wells failed for some images, return False
    """
    ...
    print(f"Found {len(image_paths)} images to crop the wells from.")
    ...
    for img_path in image_paths:
        try:
            ...
            # Run your cropper
            crop_wells(img_path, plate_type, base_dir)
    ...
    if len(failed_images) == 0:
        print("SUMMARY (CROP WELLS): Success! All wells have been cropped.")
        print("="*50)
        return True
    else:
        print(f"SUMMARY (CROP WELLS): {len(failed_images)} failures out of {len(image_paths)} images:\n")
...
if __name__ == "__main__":
    ...
    root_dir = Path(os.path.join(ROOT, "data", "cropped_data","Panama99")) # only processed images here
    base_dir = Path(os.path.join(ROOT, "data", "cropped_wells"))

    crop_wells_script(root_dir, base_dir)